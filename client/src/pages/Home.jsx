...
function Home() {
  ...
  useEffect(() => {
    const fetchUserDetails = async()=>{
      const res = await fetch('/api/user/get-user-details',{
        method: 'GET'
      });
  ...
  return (
    <div>
      <div className="custom-padding mt-10 px-10 pl-10 dark:bg-slate-900 ">
        <div className="px-4 sm:px-0 flex justify-between">
          ...
          <h2 className="text-3xl font-semibold leading-7 text-gray-900 dark:text-white">
            Medical ID:{" "}
            <span className="text-slate-600 dark:text-slate-400">{user?.medical_id}</span>
          </h2>
        </div>
        <div className="mt-6 border-t border-gray-100">
          <dl className="divide-y divide-gray-100 dark:divide-slate-800">
            <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0 text-center">
              ...
              <dd className="mt-1 text-md leading-6 text-gray-700 sm:col-span-2 sm:mt-0 dark:text-slate-200">
                {user?.name}
              </dd>
            </div>
            <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0 text-center">
      ...
      <div className=" px-10 pl-10 dark:bg-slate-900 ">
        ...
        {medicalConditions?.medical_conditions?.length > 0 && (
          <div className="relative overflow-x-auto max-w-screen-md mx-auto">
            <h3 className="text-xl font-semibold leading-7 text-gray-700 dark:text-white pt-6 pb-5">
              Condition
            </h3>
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                {medicalConditions.medical_conditions.map(
                  (condition, index) => (
                    <tr
                      key={index}
                    ...
                    >
                      ...
                      <td className="px-6 py-4">{condition.diagnosisDate}</td>
                      <td className="px-6 py-4">{condition.treatment}</td>
                    </tr>
                  )
                )}
              </tbody>
        ...
        {!(
          ...
          <p className="text-lg text-gray-700 dark:text-white mt-8 text-center ">
        ...
        <div className="flex w-0 flex-1 items-center pl-20 pt-10 ">
          <div className="ml-20 pl-20  flex justify-center mb-5">
            <Link
              to="/MedicalInfo"
              className="bg-white hover:bg-gray-100 text-gray-800 px-3 pt-1 text-sm font-semibold border border-gray-400 rounded shadow w-[100px] h-[30px]"
            >
